...
argument_parser.add_argument('--text', dest='text_file', type=str, nargs='?',help='File with the descriptions to process. Should '
                                                                               'be in csv {key,value} where key=app_name and value=description')
...
with open(arguments.text_file, 'r', encoding='utf-8') as csv_file:
    descriptions = csv.DictReader(csv_file)
...
#    ['dobj', 'advcl', 'attr', 'conj', 'appos']
    ...
    for syntactic_dependency in syntactic_dependencies:
        for row in descriptions:
            alt_document = nlp(row["description"])
            alt_chunks = []
            ...
            with open('output-data/' + row['app_name']+'_automatic_features.txt', 'a', encoding='utf-8') as file:
